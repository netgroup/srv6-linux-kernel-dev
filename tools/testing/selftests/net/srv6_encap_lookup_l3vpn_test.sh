...
# "lookup 500" to direct the post-encap SID route lookup to this table.
#
...
#
#
# Legend (concrete per-tenant addresses are in the instantiation tables below):
...
# |        | 10.0.0.201 |                                          |
# +--------+------------+------------------------------------------+
# Per tenant: the remote host and the peer router loopback, both
# dual-stack, all encapsulated to the same SID.
#
...
# Return the host id for the rlo address, unique per tenant.
# The hundreds digit comes from the tenant (100 -> 1, 200 -> 2) and the
# units digit from the router id (e.g. rt-1/tenant 100 -> "101",
# rt-2/tenant 200 -> "202"). Used for both IPv6 and IPv4.
get_rlo_hostid()
{
	local rtid="$1"
	local tid="$2"
...
# router.
# args:
#  $1 - router id (namespace resolved internally via get_rtname)
#  $2 - host part of the IPv6 destination (e.g. "2" or "101")
#  $3 - host part of the IPv4 destination (e.g. "2" or "101")
...
# Install the dual-stack encap route for a tenant host on rt, with "lookup
# 500" so the first SID is looked up in the underlay table.
# Example (rt=1, hsdst=2, sid=fc00:1:2:100::0d46, tid=100): installs
#   cafe::2/128 and 10.0.0.2/32 in vrf-100 on rt-1 with encap lookup 500.
# args:
...
# Same as set_host_encap_route() but without the lookup attribute.
...
# Install the dual-stack encap route on rt toward rtdst's rlo loopback
# (RLO1 or RLO2, see header), with "lookup 500".
# Example (rt=2, rtdst=1, sid=fc00:2:1:100::0d46, tid=100): installs
#   cafe::101/128 and 10.0.0.101/32 in vrf-100 on rt-2 with encap lookup 500.
# args:
#  $1 - router id where the encap route is installed
...
set_gw_encap_route()
{
	...
	__set_encap_route "${rt}" "${dst}" "${dst}" "${sid}" "${tid}" true
}

# Same as set_gw_encap_route() but without the lookup attribute.
set_gw_encap_route_nolookup()
{
	...
	__set_encap_route "${rt}" "${dst}" "${dst}" "${sid}" "${tid}" false
}
...
# Configure a one-way VPN path from hssrc (on rtsrc) to hsdst (on rtdst) for
# tenant tid. The encap side is set up on rtsrc and the decap side on rtdst.
# args:
#  $1 - host id of the source host
#  $2 - router id of the source router (connected to hssrc)
#  $3 - host id of the destination host
#  $4 - router id of the destination router (connected to hsdst)
#  $5 - tenant id
setup_vpn_config()
{
	local hssrc="$1"
...
# Configure rt so it can reach the loopback address of rtdst through the VPN.
# args:
#  $1 - router id where the encap route is installed
...
setup_vpn_gw_encap()
{
	...
	sid="$(build_vpn_sid "${rt}" "${rtdst}" "${tid}")"

	set_gw_encap_route "${rt}" "${rtdst}" "${sid}" "${tid}"
}
...
__test_nolookup()
{
	...
	check_hs_ipv6_connectivity "${hssrc}" "${hsdst}" "${tid}"
	log_test $? 1 "IPv6 broken w/o lookup: hs-t${tid}-${hssrc} -X-> hs-t${tid}-${hsdst} (tenant ${tid})"

	check_hs_ipv4_connectivity "${hssrc}" "${hsdst}" "${tid}"
	log_test $? 1 "IPv4 broken w/o lookup: hs-t${tid}-${hssrc} -X-> hs-t${tid}-${hsdst} (tenant ${tid})"
...
check_and_log_gw_connectivity()
{
	...
	log_test $? 0 "IPv6 output: rt-${rtsrc} -> rt-${rtdst} (tenant ${tid})"

	check_gw_ipv4_connectivity "${rtsrc}" "${rtdst}" "${tid}" "${tid}"
	log_test $? 0 "IPv4 output: rt-${rtsrc} -> rt-${rtdst} (tenant ${tid})"
}
...
check_and_log_gw_isolation()
{
	...
	log_test $? 1 "IPv6 output isolation: rt-${rtsrc} -X-> rt-${rtdst} (tenant ${tidsrc})"

	check_gw_ipv4_connectivity "${rtsrc}" "${rtdst}" "${tidsrc}" "${tiddst}"
	log_test $? 1 "IPv4 output isolation: rt-${rtsrc} -X-> rt-${rtdst} (tenant ${tidsrc})"
}

gw_vpn_isolation_tests()
{
	log_section "SRv6 VPN output path isolation test"
...
gw_vpn_tests()
{
	log_section "SRv6 VPN output path test (gateway-originated traffic)"
...
__test_gw_nolookup()
{
	...
	check_gw_ipv6_connectivity "${rtsrc}" "${rtdst}" "${tid}" "${tid}"
	log_test $? 1 "IPv6 broken output w/o lookup: rt-${rtsrc} -X-> rt-${rtdst} (tenant ${tid})"

	check_gw_ipv4_connectivity "${rtsrc}" "${rtdst}" "${tid}" "${tid}"
	log_test $? 1 "IPv4 broken output w/o lookup: rt-${rtsrc} -X-> rt-${rtdst} (tenant ${tid})"
...
{
	log_section "SRv6 VPN output path w/o lookup attribute test"
...
test_encap_lookup_supp_or_ksft_skip()
{
	...
	if ! ip -netns "${nsname}" -6 route show fc00::1/128 | \
			grep -q "lookup 254"; then
		cleanup_ns "${nsname}"
		echo "SKIP: seg6 encap lookup attribute not supported"
		exit "${ksft_skip}"
	fi
